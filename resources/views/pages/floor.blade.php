...
@section('subcontent')
            ...
            <div class="modal-content">
                ...
                <form enctype="multipart/form-data" action="{{ $ACTION }}/save-create" method="POST">
                    ...
                    <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                        <div class="col-span-12"> <label for="modal-form-1" class="form-label">Lantai</label>
                            <input name="name" id="modal-form-1" type="text" class="form-control"
                                placeholder="inputkan angka lantai">
                        </div>
                    </div> <!-- END: Modal Body -->
    ...
    @foreach ($floors as $floor)
        <div id="header-footer-modal-preview-{{ $floor->id }}" class="modal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <!-- BEGIN: Modal Header -->
                    <div class="modal-header">
                    ...
                    {{-- Form --}}
                    <form enctype="multipart/form-data" action="{{ $ACTION }}/save-edit/{{ $floor->id }}"
                        method="POST">
                        @csrf
                        <!-- BEGIN: Modal Body -->
                        <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                            <div class="col-span-12"> <label for="modal-form-1" class="form-label">Lantai</label>
                                <input name="name" id="modal-form-1" type="text" class="form-control"
                                    placeholder="inputkan angka lantai" value="{{ $floor->name }}">
                            </div>
                        </div> <!-- END: Modal Body -->
    ...
    <h2 class="intro-y text-lg font-medium mt-10">Floors</h2>
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            <button class="tooltip btn btn-primary shadow-md mr-2" title="Filter Lantai">
                <i class="w-4 h-4" data-feather="filter"></i>
            </button>
            <button class="tooltip btn btn-outline-primary shadow-md mr-2" title="Import Lantai">
            ...
            {{-- Plus --}}
            <a href="javascript:;" class="tooltip btn btn-primary shadow-md mr-2" title="Tambah Lantai" data-toggle="modal"
            ...
            <button class="disabled:opacity-50 tooltip btn text-white bg-red-300 shadow-md mr-2"
                title="Hapus Lantai yang dipilih">
                <i class="w-4 h-4" data-feather="trash-2"></i>
            </button>

            <div class="hidden md:block mx-auto text-gray-600">Showing {{ $floors->firstItem() }} to
                {{ $floors->lastItem() }} of {{ $floors->total() }} entries
            </div>
                            ...
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <a class="flex items-center mr-3" href="javascript:;" data-toggle="modal"
                                        data-target="#header-footer-modal-preview-{{ $floor->id }}">
                                        <i data-feather="check-square" class="w-4 h-4 mr-1"></i> Edit
                                    </a>
                                    <a class="flex items-center text-theme-6" href="javascript:;" data-toggle="modal"
                                        data-target="#delete-confirmation-modal-{{ $floor->id }}">
    ...
    @foreach ($floors as $floor)
        ...
        <div id="delete-confirmation-modal-{{ $floor->id }}" class="modal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-body p-0">
                        ...
                        <div class="px-5 pb-8 text-center">
                            ...
                            <a href="{{ $ACTION }}/{{ $floor->id }}" type="button"
